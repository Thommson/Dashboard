      ...
      return(
        <div id="weather-data" className="row">
          <div className="col white-text">
            ...
            <div className="row">
              <div className="inline-left">{dayNames[new Date().getDay()-1]},<br></br> {new Date().getDate()} {monthNames[new Date().getMonth()]} {new Date().getFullYear()} {new Date().getHours()}:{new Date().getMinutes()}</div>
            </div>
            ...
            <div className="row white-text">
              <div className="col-6"><FontAwesomeIcon icon={ faLongArrowAltUp } size="xs" /><FontAwesomeIcon className="stock-icon" icon={faSun} size="xs" />{new Date(Number(this.props.weatherData.find({name: "sunrise"}).attributes.state.models[0].attributes.data*1000)).getHours()}:{new Date(Number(this.props.weatherData.find({name: "sunrise"}).attributes.state.models[0].attributes.data*1000)).getMinutes()}</div>
              <div className="col-6"><FontAwesomeIcon icon={ faLongArrowAltDown } size="xs" /><FontAwesomeIcon className="stock-icon" icon={faSun} size="xs" />{new Date(Number(this.props.weatherData.find({name: "sunset"}).attributes.state.models[0].attributes.data*1000)).getHours()}:{new Date(Number(this.props.weatherData.find({name: "sunset"}).attributes.state.models[0].attributes.data*1000)).getMinutes()}</div>
            </div>
          </div>
        </div>
      )
